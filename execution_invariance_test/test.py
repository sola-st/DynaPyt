...
def remove_related_modules(project, project_module):
...
def run_tests(projects, project_module_map):
    ...
    for project in projects:
        ...
        remove_related_modules(project, project_module_map[project.name])
        ...
        for test, result in test_result.items():
            ...
            if (result != instrumented_test_result[test]):
                ...
                failed = True
                break
        print("All tests passed for project", project.name)
...
def run_test_on_projects():
    ...
    with open(test_folders_file, "r") as f:
        while True:
            ...
            if not line:
                break
            project_dir, project_module = line.strip().split()
            project = dynapyt_dir / project_dir
            projects.append(project)
            project_module_map[project.name] = project_module

    if run_tests(projects, project_module_map):

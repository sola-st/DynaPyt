...
selected_hooks = get_hooks_from_analysis("dynapyt.analyses.TraceAll", "TraceAll")


@pytest.fixture(autouse=True)
def reset():
    for dirpath, dirnames, filenames in walk(
        path.normpath(path.join(path.dirname(path.abspath(__file__)), "../test_files"))
    ):
        for f in filenames:
...
def pytest_generate_tests(metafunc):
    ...
    for dirpath, dirnames, filenames in walk(
        ...
        correct_f.extend(
            path.join(dirpath, filename + ".correct")
            for filename in filenames
            if filename.endswith(".py")
        )
    metafunc.parametrize(
        "test_file,correct_file",
        list(zip(test_f, correct_f)),
        ids=[tf.split("/")[-1][:-3] for tf in test_f],
    )
